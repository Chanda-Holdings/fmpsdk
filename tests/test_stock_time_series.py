...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            sts.historical_survivorship_bias_free_eod,
...
def test_survivorship_bias_free_data():
    ...
    result = sts.historical_survivorship_bias_free_eod(
        apikey=API_KEY, symbol="AAPL", date="2024-01-02"
    )
    assert result is not None
...
def test_different_time_intervals():
    ...
    for func, interval_name in interval_functions:
        result = func(
            apikey=API_KEY, symbol=symbol, from_date="2024-01-02", to_date="2024-01-02"
        )
...
def test_multiple_symbols():
    ...
    for symbol in symbols:
        ...
        result = sts.historical_price_eod_light(
            apikey=API_KEY, symbol=symbol, from_date="2024-01-01", to_date="2024-01-05"
        )
...
def test_return_type_verification():
    ...
    test_functions = [
        (sts.quote_short, {"symbol": "AAPL"}),
        (
            sts.historical_price_eod_full,
            {"symbol": "AAPL", "from_date": "2024-01-01", "to_date": "2024-01-05"},
        ),
        (
            sts.historical_chart_1min,
            {"symbol": "AAPL", "from_date": "2024-01-02", "to_date": "2024-01-02"},
        ),
        (sts.historical_stock_dividend, {"symbol": "AAPL"}),
    ]
