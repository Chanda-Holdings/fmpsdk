...
class TestReturnJson:
    ...
    def test_error_status_code_with_unicode_decode_error(self, mock_get):
        ...
        mock_response.content.decode.side_effect = UnicodeDecodeError('utf-8', b'\xff\xfe', 0, 1, 'invalid start byte')
        mock_get.return_value = mock_response
    ...
    def test_csv_response_with_error(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.content = b'\xff\xfe'  # Invalid UTF-8 that will cause decode error
        mock_get.return_value = mock_response
