...
def test_financial_statements_with_periods():
    ...
    # Test annual data
    result = cv.income_statement(apikey=API_KEY, symbol="AAPL", period="annual", limit=5)
    assert result is not None
    
    # Test quarterly data
    result = cv.income_statement(apikey=API_KEY, symbol="AAPL", period="quarter", limit=5)
    assert result is not None
    
    # Test balance sheet quarterly
    result = cv.balance_sheet_statement(apikey=API_KEY, symbol="AAPL", period="quarter", limit=5)
    assert result is not None
    
    # Test cash flow quarterly
    result = cv.cash_flow_statement(apikey=API_KEY, symbol="AAPL", period="quarter", limit=5)
    assert result is not None
...
def test_financial_ratios_and_metrics():
    ...
    # Test annual ratios
    result = cv.financial_ratios(apikey=API_KEY, symbol="AAPL", period="annual", limit=5)
    assert result is not None
    
    # Test quarterly ratios
    result = cv.financial_ratios(apikey=API_KEY, symbol="AAPL", period="quarter", limit=5)
    assert result is not None
    
    # Test enterprise values
    result = cv.enterprise_values(apikey=API_KEY, symbol="AAPL", period="annual", limit=5)
    assert result is not None
...
def test_historical_data_with_date_ranges():
    ...
    # Test historical DCF
    result = cv.historical_discounted_cash_flow(apikey=API_KEY, symbol="AAPL", period="annual", limit=5)
    assert result is not None
    
    # Test historical daily DCF
    result = cv.historical_daily_discounted_cash_flow(apikey=API_KEY, symbol="AAPL", limit=30)
    assert result is not None

def test_analyst_coverage():
    """Test analyst-related endpoints."""
    # Test analyst estimates
    result = cv.analyst_estimates(apikey=API_KEY, symbol="AAPL", period="annual", limit=5)
    assert result is not None
    
    # Test quarterly estimates
    result = cv.analyst_estimates(apikey=API_KEY, symbol="AAPL", period="quarter", limit=5)
    assert result is not None
    ...
    result = cv.ratings_historical(apikey=API_KEY, symbol="AAPL", from_date="2024-01-01", to_date="2024-12-31")
    assert result is not None
...
def test_earnings_and_transcripts():
    ...
    result = cv.earning_call_transcript(apikey=API_KEY, symbol="AAPL", year=2024, quarter=1)
    assert result is not None
