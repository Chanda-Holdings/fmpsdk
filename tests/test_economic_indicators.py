...
import os
import pytest
from fmpsdk.economic_indicators import (
    treasury_rates,
    economic_indicators,
)
...
def test_economic_indicators_endpoints(func, kwargs):
    ...
    if hasattr(result, 'root'):
        data = result.root
        if data is not None:
            assert isinstance(data, list), f"{func.__name__} should return RootModel with list data"
            # If there are results, check that each item is a Pydantic model
            if data:
                assert hasattr(data[0], '__dict__'), f"{func.__name__} should return list of Pydantic models"
    elif hasattr(result, '__iter__') and not isinstance(result, (str, bytes)):
        # Handle legacy list responses
        result_list = list(result)
        if result_list:
            assert hasattr(result_list[0], '__dict__'), f"{func.__name__} should return list of Pydantic models"
...
def test_economic_indicators_different_names():
    ...
    for indicator_name in indicators:
        ...
        if hasattr(result, 'root'):
            ...
            if data is not None and isinstance(data, list) and data:
                item = data[0]
                assert hasattr(item, '__dict__'), f"Failed for indicator: {indicator_name}"
