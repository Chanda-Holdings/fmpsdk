...
class TestTechnicalIndicatorsComprehensive:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("AAPL", "sma", 20, "1day", "large_cap_tech", {"range": (50, 500), "smoothing": True}),
            ("MSFT", "sma", 50, "1day", "large_cap_tech", {"range": (100, 600), "trend_following": True}),
            ("GOOGL", "sma", 10, "1day", "large_cap_tech", {"range": (80, 300), "responsive": True}),
            ("AMZN", "sma", 30, "1day", "large_cap_tech", {"range": (50, 400), "volatility": "moderate"}),
            ("TSLA", "sma", 20, "1day", "electric_vehicle", {"range": (100, 1000), "volatility": "high"}),
            
            # Exponential Moving Average tests
            ("AAPL", "ema", 12, "1day", "large_cap_tech", {"range": (50, 500), "responsive": True}),
            ("MSFT", "ema", 26, "1day", "large_cap_tech", {"range": (100, 600), "trend_following": True}),
            ("GOOGL", "ema", 9, "1day", "large_cap_tech", {"range": (80, 300), "fast_response": True}),
            
            # Financial sector stocks
            ("JPM", "sma", 20, "1day", "large_cap_financial", {"range": (50, 300), "stability": "moderate"}),
            ("BAC", "sma", 50, "1day", "large_cap_financial", {"range": (20, 100), "cyclical": True}),
            
            # Healthcare stocks
            ("JNJ", "sma", 20, "1day", "large_cap_healthcare", {"range": (100, 300), "stability": "high"}),
            ("PFE", "sma", 30, "1day", "large_cap_healthcare", {"range": (20, 80), "defensive": True}),
            
            # Energy stocks
            ("XOM", "sma", 20, "1day", "large_cap_energy", {"range": (30, 200), "cyclical": True}),
            ("CVX", "sma", 50, "1day", "large_cap_energy", {"range": (50, 300), "dividend_focused": True}),
            
            # Consumer stocks
            ("WMT", "sma", 30, "1day", "consumer_staples", {"range": (50, 300), "stability": "high"}),
            ("PG", "sma", 20, "1day", "consumer_staples", {"range": (80, 200), "defensive": True}),
            
            # ETFs
            ("SPY", "sma", 20, "1day", "broad_market_etf", {"range": (200, 700), "market_tracking": True}),
            ("QQQ", "sma", 50, "1day", "tech_etf", {"range": (150, 800), "tech_heavy": True}),
            ("IWM", "sma", 20, "1day", "small_cap_etf", {"range": (100, 400), "small_cap_focus": True}),
            ("VTI", "sma", 30, "1day", "broad_market_etf", {"range": (150, 500), "diversified": True}),
            
            # Cryptocurrencies
            ("BTCUSD", "sma", 20, "1day", "cryptocurrency", {"range": (20000, 150000), "volatility": "very_high"}),
            ("ETHUSD", "sma", 50, "1day", "cryptocurrency", {"range": (1000, 10000), "volatility": "very_high"}),
            
            # Forex
            ("EURUSD", "sma", 20, "1day", "forex", {"range": (0.8, 1.5), "volatility": "low"}),
            ("GBPUSD", "sma", 30, "1day", "forex", {"range": (1.0, 1.8), "volatility": "moderate"}),
            
            # Commodities
            ("GCUSD", "sma", 20, "1day", "commodity", {"range": (1500, 3500), "safe_haven": True}),
            ("CLUSD", "sma", 50, "1day", "commodity", {"range": (30, 200), "cyclical": True}),
        ],
    )
    def test_technical_indicators_comprehensive(
        self, api_key, symbol, indicator, period_length, timeframe, asset_class, expected_characteristics
    ):
        ...
        result, validation = handle_api_call_with_validation(
            ...
            "technical_indicators",
            allow_empty=True,  # Technical indicators may not always be available
            apikey=api_key,
            symbol=symbol,
            indicator=indicator,
        ...
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate technical indicator models for {symbol}")
        ...
        if models:
            ...
            for item in models[:5]:  # Check first 5 data points
                ...
                assert item.open is not None and item.open > 0, "Open price should be positive"
                assert item.high is not None and item.high > 0, "High price should be positive"
                assert item.low is not None and item.low > 0, "Low price should be positive"
                assert item.close is not None and item.close > 0, "Close price should be positive"
                ...
                if indicator == "sma":
                    if item.sma is not None:
                        ...
                        assert price_range[0] * 0.5 <= item.sma <= price_range[1] * 1.5, "SMA should be within reasonable price range"
                    ...
                    if item.ema is not None:
                        ...
                        price_range = (item.low, item.high)
                        assert price_range[0] * 0.5 <= item.ema <= price_range[1] * 1.5, "EMA should be within reasonable price range"
                
                # Asset class-specific validation
                if asset_class == "large_cap_tech":
                    # Tech stocks should have reasonable price ranges
                    assert expected_characteristics["range"][0] <= item.close <= expected_characteristics["range"][1], f"Close price should be within expected range for {asset_class}"
    ...
    @pytest.mark.parametrize(
        ...
        [
            # RSI tests
            ("AAPL", "rsi", 14, "1day", {"range": (0, 100), "overbought": 70, "oversold": 30}),
            ("MSFT", "rsi", 21, "1day", {"range": (0, 100), "overbought": 70, "oversold": 30}),
            ...
            ("SPY", "rsi", 14, "1day", {"range": (0, 100), "market_representative": True}),
            ("QQQ", "rsi", 14, "1day", {"range": (0, 100), "tech_heavy": True}),
            ("BTCUSD", "rsi", 14, "1day", {"range": (0, 100), "volatility": "very_high"}),
    ...
    def test_rsi_indicator_comprehensive(
        ...
        # Extract and validate models
        models = get_response_models(result, FMPTechnicalIndicator)
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate RSI indicator models for {symbol}")
        ...
        if models:
            ...
            for item in models:
                if item.rsi is not None:
                    ...
                    assert 0 <= item.rsi <= 100, "RSI should be between 0 and 100"
                    
                    # RSI behavior validation
                    if expected_behavior.get("overbought"):
                        # RSI can go above overbought levels
                        assert item.rsi <= 100, "RSI should not exceed 100"
                    
                    if expected_behavior.get("oversold"):
                        # RSI can go below oversold levels
                        assert item.rsi >= 0, "RSI should not be below 0"
    ...
    def test_williams_indicator_comprehensive(
        ...
        # Extract and validate models
        models = get_response_models(result, FMPTechnicalIndicator)
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate Williams indicator models for {symbol}")

        if models:
            for item in models:
                if item.williams is not None:
                    # Williams %R should be between -100 and 0
                    assert expected_range[0] <= item.williams <= expected_range[1], f"Williams %R should be between {expected_range[0]} and {expected_range[1]}"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("SPY", "adx", 14, "1day", {"range": (0, 100), "market_representative": True}),
            ("BTCUSD", "adx", 14, "1day", {"range": (0, 100), "volatility": "very_high"}),
    ...
    def test_adx_indicator_comprehensive(
        ...
        # Extract and validate models
        models = get_response_models(result, FMPTechnicalIndicator)
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate ADX indicator models for {symbol}")

        if models:
            for item in models:
                if item.adx is not None:
                    # ADX should be between 0 and 100
                    assert 0 <= item.adx <= 100, "ADX should be between 0 and 100"

    @pytest.mark.parametrize(
        "symbol,indicator_category,indicators,expected_relationships",
        [
            # Moving averages
            ("AAPL", "moving_averages", ["sma", "ema", "wma"], {"trend_following": True, "smoothing": True}),
            ("MSFT", "moving_averages", ["sma", "ema"], {"consistency": True}),
            ("GOOGL", "moving_averages", ["dema", "tema"], {"advanced_smoothing": True}),
            
            # Momentum oscillators
            ("AAPL", "momentum_oscillators", ["rsi", "williams"], {"momentum_signals": True}),
            ("TSLA", "momentum_oscillators", ["rsi", "williams"], {"high_volatility": True}),
            ...
            ("TSLA", "volatility_indicators", ["standarddeviation"], {"volatility_measurement": True}),
            ("BTCUSD", "volatility_indicators", ["standarddeviation"], {"high_volatility": True}),
        ],
    )
    def test_technical_indicators_category_relationships(
        ...
        for indicator in indicators:
            ...
            # Extract and validate models
            models = get_response_models(result, FMPTechnicalIndicator)
            validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate {indicator} indicator models for {symbol}")
            ...
            if models:
                for item in models:
                    ...
                    if indicator == "rsi" and item.rsi is not None:
                        ...
                        assert -100 <= item.williams <= 0, "Williams %R should be between -100 and 0"
    ...
    def test_technical_indicators_timeframe_validation(
        ...
        # Extract and validate models
        models = get_response_models(result, FMPTechnicalIndicator)
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate technical indicator models for {symbol} with timeframe {timeframe}")
...
class TestTechnicalIndicatorsValidation:
    ...
    def test_technical_indicators_data_quality(
        ...
        if models:
            ...
            for item in models:
                # OHLC validation
                if (item.open and item.high and item.low and item.close and
                    item.open > 0 and item.high > 0 and item.low > 0 and item.close > 0):
                    valid_ohlc += 1
            ...
            if total_points > 0:
                assert valid_ohlc / total_points >= 0.95, f"Only {valid_ohlc}/{total_points} points have valid OHLC data"
                assert valid_dates / total_points >= 0.95, f"Only {valid_dates}/{total_points} points have valid dates"
                if indicator in ["sma", "ema", "rsi", "williams", "adx"]:
                    assert valid_indicators / total_points >= 0.80, f"Only {valid_indicators}/{total_points} points have valid {indicator} values"
    ...
    def test_technical_indicators_period_effects(
        ...
        # Extract and validate models
        models = get_response_models(result, FMPTechnicalIndicator)
        validate_model_list(models, FMPTechnicalIndicator, f"Failed to validate SMA indicator models for {symbol} with period {period_length}")
        ...
        if models:
            ...
            if len(sma_values) >= 2:
                ...
                if expected_smoothing == "very_high_smoothing":
                    ...
                    assert volatility < 0.1, f"Very high smoothing should have low volatility, got {volatility}"
                elif expected_smoothing == "high_smoothing":
                    # High smoothing should have moderate volatility
                    assert volatility < 0.2, f"High smoothing should have moderate volatility, got {volatility}"
                elif expected_smoothing == "moderate_smoothing":
                    # Moderate smoothing should have moderate volatility
                    assert volatility < 0.3, f"Moderate smoothing should have moderate volatility, got {volatility}"
                elif expected_smoothing == "low_smoothing":
                    # Low smoothing should have higher volatility
                    assert volatility < 0.5, f"Low smoothing should have higher volatility, got {volatility}"
