"""
Integration tests for fmpsdk calendar_module endpoints.

This module tests the calendar and event functionality including:
- Historical dividend data for individual stocks
- Dividend calendar events (upcoming dividends)
- Earnings calendar events (upcoming earnings)
- IPO calendar and related filings
- Stock split data and calendar events
- Response schema validation
- Error handling
- Date range filtering and parameter validation
"""
...
def extract_data_list(result):
    """Helper function to extract data list from result."""
...
class TestDividends:
    ...
    def test_dividends_chronological_order(self, api_key):
        ...
        result = dividends(apikey=api_key, symbol="KO", limit=10)  # Coca-Cola has long dividend history
        ...
        assert is_ascending or is_descending, "Dividends should be in chronological order"
...
class TestDividendsCalendar:
    ...
    def test_dividends_calendar_current_month(self, api_key):
        ...
        to_date = (today.replace(day=28) + timedelta(days=4)).replace(day=1).strftime('%Y-%m-%d')
        
        result = dividends_calendar(apikey=api_key, from_date=from_date, to_date=to_date)
    ...
    def test_dividends_calendar_specific_date_range(self, api_key):
        ...
        # Validate date range if data is available
...
class TestEarningsCalendar:
    ...
    def test_earnings_calendar_earnings_season(self, api_key):
        ...
        if len(result_list) > 0:
            ...
            assert len(set(symbols)) > 5, "Should have multiple companies during earnings season"
...
class TestIPOCalendar:
    ...
    def test_ipos_calendar_current_quarter(self, api_key):
        ...
        if quarter_start.month == 10:
            ...
            quarter_end = datetime(today.year, next_quarter_month, 1) - timedelta(days=1)
...
class TestStockSplits:
    ...
    def test_splits_valid_symbol(self, api_key):
        ...
        assert validated.ratio is not None
    ...
    def test_splits_with_limit(self, api_key):
        ...
        if len(result_list) > 0:
            first_item = result_list[0]
            symbol_value = get_field_value(first_item, 'symbol')
            ratio_value = get_field_value(first_item, 'ratio')
    ...
    def test_splits_all_recent(self, api_key):
        ...
        result = splits(apikey=api_key, limit=10)
...
class TestSplitsCalendar:
    ...
    def test_splits_calendar_current_year(self, api_key):
        ...
        if len(result_list) > 0:
            first_item = result_list[0]
            if isinstance(first_item, dict):
                validated = FMPStockSplitCalendarEvent.model_validate(first_item)
            else:
                validated = first_item
    ...
    def test_splits_calendar_specific_period(self, api_key):
        """Test splits calendar for a specific period with known splits."""
        # 2020 had several notable stock splits
        ...
        # 2020 had Apple and Tesla splits among others
        if len(result_list) > 0:
            symbols = [get_field_value(item, 'symbol') for item in result_list]
            # Check if we have known splits from that period
            known_2020_splits = ["AAPL", "TSLA"]
            has_known_splits = any(symbol in symbols for symbol in known_2020_splits)
            # This assertion may fail if historical data is limited
            # assert has_known_splits, f"Should include known 2020 splits, got symbols: {symbols[:10]}"
...
class TestCalendarDataConsistency:
    ...
    def test_dividend_dates_consistency(self, api_key):
        ...
        for item in result_list:
            validated = FMPDividend.model_validate(item) if isinstance(item, dict) else item
            
            # Declaration date should be before record date
            if validated.declarationDate and validated.recordDate:
                assert validated.declarationDate <= validated.recordDate, \
    ...
    def test_split_ratio_consistency(self, api_key):
        ...
        if len(result_list) > 0:
            for item in result_list:
                validated = FMPStockSplit.model_validate(item) if isinstance(item, dict) else item
                ...
                # Ratio should match numerator/denominator calculation
                expected_ratio = validated.numerator / validated.denominator
                assert abs(validated.ratio - expected_ratio) < 0.001, \
                ...
                assert validated.ratio != 1.0, "Split ratio should not be 1:1"
    ...
    def test_calendar_date_ranges(self, api_key):
        ...
        # Validate that all returned dates are within the requested range
        for item in result_list:
            date_value = get_field_value(item, 'date')
            assert from_date <= date_value <= to_date, \
                f"Date {date_value} should be within range {from_date} to {to_date}"
